...
static t_dbls	*get_dbl_arg_val(t_dbls *arg_val, t_parser *conv_params,
                                  va_list args)
{
    if (conv_params->size == CAP_L)
    {
        if (!(arg_val->ldbl = (long double)va_arg(args, long double)))
            conv_params->flags |= NFL;
...
char			*iffloat(t_parser *format, va_list args)
{
    t_dbls		arg_val;
    char		*val_str;
